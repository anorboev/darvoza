...
| 4b | **Rogue caller → DENIED at the call** (2:00–2:30) | Terminal: `dotnet run --project demo/tools/RogueCaller` → `Policy denied: tool 'wit_create_work_item' is not permitted.`; cut to the audit `tail` showing the new line | "But policy isn't a client-side suggestion. This script skips `tools/list` and invokes the tool directly — what a compromised client would do. Denied at the call, *before* it ever reaches Azure DevOps. Audit line: `decision: deny`, role analyst, upstream null. The org never saw the request." |
